...
namespace exl::patch {
    ...
    class StreamPatcher : public PatcherImpl {
        ...
        template<typename T>
        inline ptrdiff_t RelativeAddressFromPointer(T ptr) const {
        ...
        /* Absolute address. */
        template<typename T>
        inline void Seek(T ptr) {
            SeekRel(RelativeAddressFromPointer(ptr));
        }

        template<typename T>
        inline void SeekWithOffset(uintptr_t address, uintptr_t offset) {
            SeekRel(RelativeAddressFromBase(address + offset));
        }

        template<typename T>
        inline void SeekWithOffset(T ptr, uintptr_t offset) {
            void* addr = (void*)(((char*)ptr) + offset);

            SeekRel(RelativeAddressFromPointer(addr));
        }
