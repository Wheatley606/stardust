...
namespace exl::patch {
    ...
    class PatcherImpl {
        ...
        template<typename T>
        inline T& AtRo(const uintptr_t offset) {
            /* Get address for the object. */
            uintptr_t start = RoFromAddr(offset);
        ...
        template<typename T>
        inline ptrdiff_t AddrFromRoPointer(T ptr) const { return AddrFromRo(reinterpret_cast<uintptr_t>(ptr)); }

        template<typename T>
        inline ptrdiff_t AddrFromRwPointer(T ptr) const { return AddrFromRw(reinterpret_cast<uintptr_t>(ptr)); }
    };
}