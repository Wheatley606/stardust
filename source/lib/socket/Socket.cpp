...
{
    return gSocket.init(ip, port);
}

void logger::log(std::string message)
{
    if (!gSocket.isConnected()) return;

    gSocket.sendMessage(message.c_str());
}
...
void Socket::sendMessage(const char *message) {
    ...
    size_t offset = 0;
    while (offset < length) {
        size_t minSize = length - offset < PACKET_MAX_SIZE ? length - offset : PACKET_MAX_SIZE;
